...
function _fzf_preview_file --description "Print a preview for the given file based on its file type."
    ...
    if test -L "$file_path" # symlink
        ...
        set mimetype (file --brief --dereference --mime -- "$file_path")
        if string match -q "image*" "$mimetype"
            set dim "$FZF_PREVIEW_COLUMNS"x"$FZF_PREVIEW_LINES"
            if test "$dim" = "x"
                set dim (stty size < /dev/tty | awk '{print $2 "x" $1}')
            end
            kitty icat --clear --transfer-mode=memory --unicode-placeholder --stdin=no --place="$dim@0x0" "$file_path" | sed '$d' | sed '$s/\$/\e[m/'
